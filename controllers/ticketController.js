...
class TicketController {
  async create(req, res) {
    try {
      ...
      if (!ticketData.assigned_to) {
        const availableTechnicians = await Technician.findAll({ is_available: true });
        if (availableTechnicians.length > 0) {
          ...
          const techniciansWithWorkload = await Promise.all(
            availableTechnicians.map(async (tech) => {
              ...
              return { ...tech, activeTickets: workload.open_tickets + workload.in_progress_tickets };
            })
          );
          
          techniciansWithWorkload.sort((a, b) => a.activeTickets - b.activeTickets);
          if (techniciansWithWorkload[0].activeTickets < techniciansWithWorkload[0].max_tickets) {
  ...
  async getAll(req, res) {
    try {
      const filters = {
        ...
        assigned_to: req.query.assigned_to || (req.user.role === 'technician' ? req.user.id : undefined),
  ...
  async update(req, res) {
    try {
      ...
      const updatedTicket = await Ticket.update(req.params.id, req.body, req.user.id);
