...
class KnowledgeBase {
  static async create(articleData) {
    ...
    const sql = `INSERT INTO knowledge_base (title, content, category_id, tags, created_by) 
                 VALUES (?, ?, ?, ?, ?)`;
    const result = await query(sql, [title, content, category_id || null, tags || null, created_by]);
  ...
  static async findById(id) {
    ...
    if (results[0]) {
      // Incrementar vistas
      await query('UPDATE knowledge_base SET views = views + 1 WHERE id = ?', [id]);
      results[0].views += 1;
    }
  ...
  static async incrementHelpful(id) {
    await query('UPDATE knowledge_base SET helpful_count = helpful_count + 1 WHERE id = ?', [id]);
    return await this.findById(id);
  }
